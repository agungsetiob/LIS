...
class CetakItem extends pdf
{
	function Header()
	{
		...
		$this->Ln(10);
		$this->Cell(0,5,'LAPORAN BERDASARKAN ITEM PARAMETER',0,0,'C');
	...
	function Report($awal,$akhir,$item)
	{
		...
		$sql = "SELECT a.nomor, c.nama, c.no_rm, c.gender, c.tgl_lahir, c.state, a.id_dokter, a.id_petugas FROM periksa a LEFT JOIN result b ON a.nomor=b.KodePatient LEFT JOIN pasien c ON a.id_pasien=c.id WHERE DATE(a.tanggal) BETWEEN '$awal' AND '$akhir' AND a.state='1' AND b.acc='1' AND b.KodeParamater='$kode->lis' ORDER BY a.nomor";
		$no=1;
		$grandtotal=0;
