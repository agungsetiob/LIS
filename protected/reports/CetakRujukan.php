<?php
class CetakRujukan extends pdf
{
	function Header()
	{
		...
		$this->Ln(10);
		$this->Cell(0,5,'LAPORAN RUJUKAN PASIEN',0,0,'C');

		$this->setFont('Arial','',16);

		$this->Ln(7);
		$this->Cell(0,5,Dokter::item($this->supplier),0,0,'C');
		...
		$this->Cell(109,5,'Rujukan Dokter',1,0,'C');
	...
	function Report($awal,$akhir,$dokter)
	{
		$this->Ln(5);
		$this->setwidths(array(8,28,55,10,20,109,35,41,40));
		...
		$sql = "SELECT a.nomor, b.no_bpjs, b.nama, b.gender, b.tgl_lahir, a.id_dokter, a.id_petugas, d.nama AS parameter, c.Nilai, d.pembulatan FROM periksa a LEFT JOIN pasien b ON a.id_pasien=b.id LEFT JOIN result c ON a.nomor=c.KodePatient LEFT JOIN kode d ON c.KodeParamater=d.lis WHERE DATE(a.tanggal) BETWEEN '$awal' AND '$akhir' AND a.id_dokter='$dokter' AND c.acc='1' AND a.state='1'  ORDER BY a.nomor";
