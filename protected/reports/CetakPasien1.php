<?php
class CetakPasien1 extends fpdf
{
	function Header()
	{
		...
		$this->Cell(3.3,0.5,'Instansi',1,0,'C');
	...
	function Report($awal,$akhir,$instansi)
	{
		$awal = Parameter::tglMySQL($awal);
		$akhir = Parameter::tglMySQL($akhir);

		$sql = "SELECT a.nomor, b.id_instansi, b.gender, b.nama, b.gender, b.tgl_lahir, a.id_dokter, a.id_petugas FROM periksa a LEFT JOIN pasien b ON a.id_pasien=b.id WHERE DATE(tanggal) BETWEEN '$awal' AND '$akhir' AND a.state='1' AND b.state='3'";
		
		if($instansi!=0){
			$sql .= " AND b.id_instansi='$instansi'";
		}
		...
		foreach ($model as $data){
			...
			$this->Cell(3.3,0.5,substr(Instansi::item($data['id_instansi']),0,17),'LR',0,'L');
		...
		// $this->Ln(2);
		// $this->Cell(0.3,0.5,'1',1,0,'C');
		// 0.3 == 1 huruf substr("Hello world",0,10)
	}
}
?>